...
//        G4ThreeVector(45 * cm, 45 * cm, 0 * cm) //50*****
  ...
  new G4PVPlacement(nullptr,  // no rotation
    ...
    G4ThreeVector positions2[16] = {
    	G4ThreeVector(-0.3 * cm, 0 * cm, 0 * cm), //1

        G4ThreeVector(-0.9 * cm, 0 * cm, 0 * cm), //2

        G4ThreeVector(-1.5 * cm, 0 * cm, 0 * cm), //3

        G4ThreeVector(-2.1 * cm, 0 * cm, 0 * cm), //4

        G4ThreeVector(-2.7 * cm, 0 * cm, 0 * cm), //5

        G4ThreeVector(-3.3 * cm, 0 * cm, 0 * cm), //6

        G4ThreeVector(-3.9 * cm, 0 * cm, 0 * cm), //7

        G4ThreeVector(-4.5 * cm, 0 * cm, 0 * cm), //8*****
    ...
    auto logicPlaca = new G4LogicalVolume(solidPlacas, placa_mat, "placas");
  ...
  // placas de elemento combustible de control
    ...
    auto logicPlaca2 = new G4LogicalVolume(solidPlacas2, placa_mat, "placas2");
  ...
  G4Material* detector_mat = nist->FindOrBuildMaterial("G4_He");
